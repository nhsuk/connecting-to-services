...
function parseOverview(xml) {
  ...
  xmlParser.parseString(xml, (err, result) => {
    ...
    let orgDetails;
    try {
      orgDetails = result.feed;
      const allOpeningTimes =
        orgDetails.entry[0].content[0].overview[0].openingTimes[0].timesSessionTypes[0];
      ...
      const id = orgDetails.link.map((link) => {
        if (link.$.rel === 'self') {
          return link.$.href;
        }
        return null;
      ...
      throw new Verror(e, `Unable to get general opening times from xml for '${id}'`);
    }
  });
